...
export default function CartPage() {
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            if (success) {
                // Optional: navigate or show success confetti handled in context
                navigate('/orders');
            }
        } catch (error) {
            console.error(error);
            ...
            <div className="fixed bottom-20 md:bottom-24 left-0 right-0 px-4 md:static md:p-0 z-10">
                <div className="bg-white p-5 rounded-3xl shadow-[0_-4px_20px_rgba(0,0,0,0.05)] md:shadow-sm border border-gray-50 max-w-lg mx-auto">
                    ...
                    <button
                        ...
                        className="w-full bg-love-600 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-love-700 transition-colors shadow-lg shadow-love-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isPurchasing ? 'İşleniyor...' : <>Satın Al <ArrowRight size={20} /></>}
                    </button>
                </div>
            </div>

            <ProcessingModal isOpen={isPurchasing} message="Lütfen bekle aşkım..." />
        </div>
    );
}
